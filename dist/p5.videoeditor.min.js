!(function (e, t) {
  'object' == typeof exports && 'undefined' != typeof module
    ? t(exports, require('gif.js/src/GIF'))
    : 'function' == typeof define && define.amd
      ? define(['exports', 'gif.js/src/GIF'], t)
      : t(
  ...
  class r {
    ...
    async render(e, t, s) {
      this._renderScene(e, t, s);
      const i = await this._applyPostProcessing();
      (this.p.image(i, 0, 0), (this.postProcessingEffects = []));
    }
    _renderScene(e, t, s) {
      ...
      for (const e of r) {
        ...
        e.render(this.sceneBuffer);
      }
      for (const e of t) e.render(this.sceneBuffer, s);
      this.sceneBuffer.pop();
    }
    async _applyPostProcessing() {
      if (0 === this.postProcessingEffects.length) return this.sceneBuffer;
      let e = this.sceneBuffer,
        t = this.effectBuffer;
      for (let s = 0; s < this.postProcessingEffects.length; s++) {
        const i = this.postProcessingEffects[s];
        this.shaderPromises[i.type] && (await this.shaderPromises[i.type]);
        const r = this.shaders[i.type];
        if (r) {
          if ((t.shader(r), r.setUniform('u_texture', e), i.uniforms))
            for (const [e, t] of Object.entries(i.uniforms)) r.setUniform(e, t);
          (t.rect(-this.width / 2, -this.height / 2, this.width, this.height),
            ([e, t] = [t, e]));
        }
      }
      return e;
  ...
  class n {
    constructor(e, t, { duration: s = 1e4 } = {}) {
      ((this.duration = s),
        (this.clips = []),
        (this.transitions = []),
        (this.time = 0),
        ...
        (this.renderEngine = new r(e, t)),
        (this.pluginManager = new i()),
    ...
    addTransition(e) {
      ...
      if (!t) return (o.warning(`Unknown transition type: ${e.type}`), null);
      const s = new t(e);
    ...
    getActiveClips() {
      ...
      for (const t of this.clips)
        this.time >= t.start && this.time < t.start + t.duration && e.push(t);
      return e;
    }
    _getFrameState(e) {
      const t = new Set(),
      ...
      for (const s of this.clips)
        e >= s.start && e < s.start + s.duration && t.add(s);
      for (const i of this.transitions)
        e >= i.start &&
          e < i.start + i.duration &&
          (s.push(i), t.add(i.fromClip), t.add(i.toClip));
      return { clipsToProcess: t, activeTransitions: s };
    }
    update(e) {
      ...
      const { clipsToProcess: t } = this._getFrameState(this.time);
      for (const s of t) {
    ...
    async render() {
      const { clipsToProcess: e, activeTransitions: t } = this._getFrameState(
        this.time
      );
      await this.renderEngine.render(e, t, this.time);
    }
    play() {
      this.isPlaying = !0;
    }
    ...
    _loadPlugins() {
      for (const e of this.pluginManager.plugins)
        try {
          ...
          o.critical(`Error loading plugin: ${e.name}`, t);
        }
    }
  }
  class a {
  ...
  class h {
    constructor({ onProgress: e, onLog: t, onError: i, onComplete: r } = {}) {
  ...
  class l {
    constructor(e, t, s) {
      ((this.timeline = e),
        (this.canvas = t),
        (this.container = s),
        (this.frameRecorder = new a(this.canvas)),
        (this.exporter = new h({
  ...
  class f {
    ...
    addEffect(e = {}) {
      ...
      if (s) {
        const t = new s(e);
        this.effects.push(t);
      } else o.warn(`Effect with type "${t}" not found.`);
      return this;
    }
    finalizeChanges() {
    ...
    _calculateValue(e, t, s) {
      ...
      const a = i[n],
        ...
        c = (u[a.easing] || u.linear)(l),
        p = a.value,
        d = h.value;
      return e.Color && p instanceof e.Color && d instanceof e.Color
        ? e.lerpColor(p, d, c)
        : e.lerp(p, d, c);
    }
    render(e) {
  ...
  const C = ['http:', 'https:', 'blob:', 'data:'];
  class E {
    constructor(e = {}) {
      ((this.x = e.x || 0),
        (this.y = e.y || 0),
        (this.width = e.width || 100),
        (this.height = e.height || 50),
    ...
    isHit(e, t) {
      const s = this.width / 2,
        i = this.height / 2;
      return (
        e >= this.x - s && e <= this.x + s && t >= this.y - i && t <= this.y + i
      );
    }
  }
  class x extends f {
    ...
    update(e, t) {
      (this._initElement(), super.update(e, t));
      const s = t / 1e3;
      ...
      if (i && !this.isPlaying) {
        ...
        (void 0 !== e && e.catch(() => {}), (this.isPlaying = !0));
      } else
        !i &&
          this.isPlaying &&
    ...
    addHotspot(e = {}) {
      return (this.hotspots.push(new E(e)), this);
    }
    checkClick(e, t, s, i) {
      const r = t - this.properties.x,
        o = s - this.properties.y;
      for (const e of this.hotspots) {
        if (i >= e.start && i < e.start + e.duration && e.isHit(r, o))
          return (e.onClick(), !0);
      }
      return !1;
    }
  ...
  class b extends f {
    ...
    update(e, t) {
      super.update(e, t);
      const s = this.slides[this.currentSlideIndex];
      if (s) {
        const i = t - this.slideActivationTime;
        s.forEach((t) => {
          t.update(e, i);
        });
      }
    ...
    render(e) {
      super.render(e);
      const t = this.slides[this.currentSlideIndex];
      if (t) {
        [...t]
  ...
  ((e.AudioClip = w),
    ...
    (e.ImageClip = y),
    (e.MemoryManager = p),
    (e.PerformanceManager = c),
    (e.PlaybackController = l),
    (e.ShapeClip = g),
    (e.SlideShowClip = b),
    (e.TextClip = m),
    (e.Timeline = n),
    (e.VideoClip = x),
    (e.VideoEditor = class {
      static ErrorHandler = o;
      constructor(
        e,
        {
          canvas: t = null,
          uiContainer: s = null,
          gifWorkerPath: i = './gif.worker.js',
          ...r
        } = {}
      ) {
        if (!e)
        ...
        ((this.options = { gifWorkerPath: i }),
          (this.timeline = new n(e, t, r)),
          (this.playbackController = new l(this.timeline, t, s)),
          (this.performanceManager = new c(r.performance)),
      ...
      async exportGIF({
        frameRate: e = 15,
        quality: s = 10,
        filename: i = 'p5.videoeditor-export.gif',
        onProgress: r = null,
      } = {}) {
        console.log('Starting GIF export...');
        const o = this.playbackController.isPlaying,
          n = this.timeline.time;
        ...
        const a = new t({
          ...
          h = 1e3 / e,
          l = Math.floor(this.timeline.duration / h);
        for (let e = 0; e < l; e++) {
          const t = e * h;
          (this.seek(t),
            this.update(this.timeline.p),
            await this.render(),
            a.addFrame(this.timeline.canvas, { copy: !0, delay: h }),
            r && r((e + 1) / l));
        }
        (a.on('finished', (e) => {
          const t = URL.createObjectURL(e),
      ...
      createSlideShowClip(e = {}) {
        const t = new b(e);
        return (this.timeline.addClip(t), t);
      }
      createTextClip(e, t = {}) {
        const s = new m(e, t);
        return (this.timeline.addClip(s), s);
      ...
      handleMousePressed(e) {
        const t = this.timeline.getActiveClips();
        for (let s = t.length - 1; s >= 0; s--) {
          const i = t[s];
          if (i instanceof x) {
            const t = this.timeline.time - i.start;
            if (i.checkClick(e, e.mouseX, e.mouseY, t)) break;
          }
        }
      }
      showUserFriendlyError(e) {
        o.showUserFriendlyError(e);
      }
    }));
});
